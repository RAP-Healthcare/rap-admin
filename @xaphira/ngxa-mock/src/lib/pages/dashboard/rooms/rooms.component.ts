...
@Component({
  ...
  template: `
    <nb-card [size]="breakpoint.width >= breakpoints.sm ? 'giant' : ''">
      ...
      <xa-room-selector [class.dark-background]="isDarkTheme" (select)="select($event)"></xa-room-selector>
      <xa-player [collapsed]="isCollapsed() && breakpoint.width <= breakpoints.md"></xa-player>
    </nb-card>
  `,
})
