...
@Component({
  selector: 'xa-country-orders',
  styleUrls: ['./country-orders.component.scss'],
  template: `
    <nb-card [size]="breakpoint.width >= breakpoints.md ? 'medium' : 'giant'">
      <nb-card-header>Country Orders Statistics</nb-card-header>
      <nb-card-body>
        <xa-country-orders-map (select)="selectCountryById($event)"
                                countryId="USA">
        </xa-country-orders-map>
        <xa-country-orders-chart [countryName]="countryName"
                                  [data]="countryData"
                                  [labels]="countriesCategories"
                                  maxValue="20">
        </xa-country-orders-chart>
      </nb-card-body>
    </nb-card>
  `,
})
